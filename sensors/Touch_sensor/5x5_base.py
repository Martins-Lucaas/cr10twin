...
# Periodo minimo entre redesenhos (s); com blitting da pra ir ate ~30 Hz.
PLOT_PERIOD = 0.03
...
# Tempos guardados em time.monotonic() (chegada real), nao no timestamp do
# firmware, para nao depender do relogio do STM32 (ADC e spikes tem bases
# diferentes e isso causava limpezas indevidas da janela deslizante).
...
# Artistas dinamicos marcados animated=True ficam fora do draw() normal;
# cada frame so repinta eles via blitting.
...
# ---- RASTER FA / SA ----
# Tempo relativo (0 = agora); limites fixos p/ blitting simples.
# SA embaixo (1..25), FA em cima (26..50), como no artigo.
...
# Fundo estatico capturado uma vez; recapturado (_bg=None) ao redimensionar.
_bg = None
...
while (time.time() - start_time) < RECORD_TIME:
    ...
    # Atualiza os graficos no maximo a cada PLOT_PERIOD.
    now = time.time()
...
# Desliga animated e redesenha: senao a janela final so mostraria o fundo
# cacheado, sem os artistas dinamicos, ate o usuario fechar.
