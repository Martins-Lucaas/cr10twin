...
# No Windows a porta varia (COMx), então o default fica None (autodetecção
# via detect_serial_port); no Linux mantém /dev/ttyACM1.
...
def note_time(t):
    """Avança current_time e reancora tudo se o micros() do STM32 regredir
    (reset ou wrap ~71 min) — senão a poda por (now_t - t) nunca descartaria
    spikes antigos."""
    global current_time
...
def process_line(line):
    """Roda na thread de leitura, não no matplotlib, para a retransmissão UDP
    não depender da janela estar respondendo. O firmware não emite um total
    escalar; ele é sintetizado aqui por frame, quando chega o último taxel."""
...
def serial_reader_loop():
    """Thread separada do matplotlib: o fluxo UDP p/ ROS não trava se o
    desenho engasgar, e dá pra detectar serial muda mesmo com janela ociosa."""
    buf = ""
...
def update(frame):
    ...
    # Número usa a MESMA matriz rotacionada (vr) que a imagem, senão diverge.
    vr = np.rot90(vm, 2)
    im_volt.set_data(vr)
    for r in range(ROWS):
        for c in range(COLS):
    ...
    # Avisa quando a serial fica muda.
    if rx_age is None:
