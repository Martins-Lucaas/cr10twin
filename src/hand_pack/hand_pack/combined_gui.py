...
# Limites factíveis derivados do manual técnico da COVVI Hand (ver
# hand_pack/urdf_helpers.py para o detalhamento). DEVEM casar com
# HAND_DRIVER_LIMITS / HAND_DRIVER_LOWER em urdf_helpers.py.
...
# Poses de pick — ângulos em graus calculados via IK do grasp_ml_pack para
# os 3 objetos farmacêuticos na estação de pick (x=0.75, y=0).
PICK_POSES = {
    'Pick Frasco (z=0.866)': {
...
HAND_GRIPS_FRAC = {
    ...
    # pick-and-place em grasp_ml_pack/poses.py).
    'Claw Grip (tubo)': {
...
class CombinedControlGUI(Node):
    ...
    def _arm_apply_preset(self, preset):
        # Atualiza os sliders SEM disparar callbacks individuais (que
        # poderiam publicar 6 trajetórias separadas e gerar trancos).
    ...
    def _build_hand_panel(self, parent):
        ...
        # Configurações de preensão para os 3 objetos farmacêuticos. Aplica
        # HAND_CONFIGS do projeto, idêntico ao que o executor automático usa
        # em ciclo.
    ...
    def _hand_apply_grip(self, vals: dict, label: str | None = None):
        ...
        # (feedback de "qual configuração foi pedida").
