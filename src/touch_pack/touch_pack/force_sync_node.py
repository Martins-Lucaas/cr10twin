"""
force_sync_node.py — Pareia em tempo real a célula de carga (XIAO,
serial USB via force_receiver) com o touch sensor (STM32 → PC plotter,
UDP 8081).

Tópico publicado:
  /touch_sync/data   touch_pack_msgs/SyncedTouch   par sincronizado

Estratégia de sincronização: as duas fontes publicam Float32 sem stamp e são
carimbadas AQUI, no mesmo relógio do PC do ROS — os instantes de chegada são
diretamente comparáveis, já que os dois transportes (serial USB da célula,
UDP de LAN do toque) têm latência muito menor que o período de amostragem da
célula. O par é emitido quando as duas amostras são frescas
(< SYNC_MAX_AGE_S); as idades vão na mensagem para auditoria posterior.
"""
...
# Teto de emissão. O par só é tão fresco quanto a fonte MAIS LENTA (a
# célula, 10-80 Hz conforme o RATE do HX711).
SYNC_RATE_HZ    = 100.0   # teto do republish (nunca acima disto)
WATCHDOG_RATE_HZ = 20.0   # timer de segurança quando não há amostra nova
...
class ForceSyncNode(Node):
    ...
    def __init__(self):
        ...
        # grandeza que o explorer e o display da GUI consomem.
        ...
        self._min_period_s = 1.0 / SYNC_RATE_HZ
        self._last_emit_s = 0.0
        self.create_timer(1.0 / WATCHDOG_RATE_HZ, self._emit)

        self.get_logger().info(
            f'ForceSync: /load_cell/force_net + /touch_sensor/value → '
            f'/touch_sync/data (1 msg por amostra da célula, teto '
            f'{SYNC_RATE_HZ:.0f} Hz, watchdog {WATCHDOG_RATE_HZ:.0f} Hz, '
    ...
    def _on_load_cell(self, msg: Float32) -> None:
        ...
        # Amostra nova da fonte lenta = a única hora em que o par carrega
        # informação nova. Emite já, sem esperar o watchdog.
        self._emit()
    ...
    def _emit(self) -> None:
        now = self._now_s()
        # Teto de taxa: protege contra uma célula que dispare acima de
        # SYNC_RATE_HZ e contra o watchdog colidir com a amostra.
        if now - self._last_emit_s < self._min_period_s:
            return
        with self._lock:
