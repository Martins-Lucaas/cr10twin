...
API pública:
    ...
    rot_error(R_curr, R_des)        → vetor de rotação (3,) no frame base
...
# TCP de palpação: pilha da célula de 6 eixos montada no flange. Geometria
# lida de `cad/step/MONTAGEM_FA7155_stack.step` (Z=0 na face do flange do
# CR10, +Z saindo dela):
#
#     0,0 …   6,0   flange fixo FA7155        (⌀75, comprado à parte)
#     0,4 …  36,9   célula FA7155 de 6 eixos  (⌀75, liga de alumínio)
#    37,2 …  59,2   acoplador_celula_hotswap  (disco ⌀66 + haste)
#    49,7 …  67,7   ponteira F                (luva + cabeça 15,4 × 17,4)
#
# 18/08/2026: era 162,2 mm com a pilha da célula axial de 100 kg (viga S +
# touch_tool + ponteira D com sensor 5×5). A célula de 6 eixos é 94,5 mm mais
# curta — poses ensinadas com a ferramenta antiga NÃO valem mais.
T_TOUCH_TOOL_ATTACH = np.array([
    [1.0,  0.0,  0.0,  0.0000],
    [0.0,  1.0,  0.0,  0.0000],
    [0.0,  0.0,  1.0,  0.0677],  # +67.7 mm — flange → face da ponteira
    [0.0,  0.0,  0.0,  1.0000],
], dtype=float)

# Limites articulares — convenção URDF (rad). Joints 2 e 4 têm offset de
# -π/2 em relação à convenção DH; os limites físicos são mapeados de ±170°
# (DH) → [-260°, +80°] (URDF).
...
# Alias público: a IK diferencial da viewport 3D (manip3d) precisa do mesmo
# erro de rotação que a IK completa usa, sem duplicar a fórmula.
rot_error = _rot_error
...
# direto do URDF (joints `*_proximal_j_input_joint`).
...
# Mimic joints da mão COVVI 26 juntas escravas extraídas de
...
# Conversão URDF ↔ DOBOT joint1 do URDF tem axis "0 0 -1" (gira em −Z), mas
# o J1 do CR10 real positivo gira em +Z → sinal INVERTIDO na junta 1
# (verificado empiricamente: sem o flip, sim e real giram a base em direções
# opostas). joints 2–6 têm a mesma convenção nos dois lados.
